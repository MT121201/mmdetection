...
def tta_inference(model, img_path):
    ...
    for scale in SCALES:
        for flip in FLIPS:
            # Clone and patch pipeline for current TTA mode
            tta_cfg = copy.deepcopy(model.cfg)
            for t in tta_cfg.test_pipeline:
    ...
    for cls in labels.unique():
        ...
        keep_idx = nms(cls_boxes, cls_scores, iou_threshold=IOU_THRESH)
...
def main(config_file, checkpoint_file, test_dir, output_txt, device='cuda:0'):
    ...
    with open(output_txt, 'w') as out_f:
        for fname in tqdm(images, desc='TTA Inference'):
            ...
            for label, bbox, score in zip(labels, bboxes, scores):
                if score < CONF_THRESH:
                    continue
                label = int(label) + 1  # 1-based class ID
...
if __name__ == '__main__':
    ...
    parser.add_argument('--output_txt', type=str, default='predictions.txt')
